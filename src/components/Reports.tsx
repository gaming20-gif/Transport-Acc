...
export const Reports: React.FC<ReportsProps> = ({ vehicles, transactions }) => {
  ...
  const [roundChartMode, setRoundChartMode] = useState<'expenses' | 'vehicles' | 'cashflow'>('expenses');
  ...
  // Get data for charts (group by month for the last 12 months, current month first)
  const getChartData = () => {
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    const monthlyData: { [key: string]: { month: string; income: number; expense: number; pending: number } } = {};
    const keysOrder: string[] = [];

    // Initialize last 12 months starting from current month (i = 0) down to 11 months ago
    const now = new Date();
    for (let i = 0; i < 12; i++) {
    ...
    return keysOrder.map(key => monthlyData[key]);
  };

  const chartData = getChartData();

  // Get data for expenses category donut chart
  const getExpenseBreakdownData = () => {
    const txsToUse = selectedVehicleId ? (filteredTxs.length > 0 ? filteredTxs : vehicleTxs) : transactions;
    ...
    const colors = ['#D96A4B', '#964A2F', '#B35E46', '#E28766', '#8C341D', '#5A4540', '#A68D85'];
    
    const data = Object.keys(categoryTotals).map((cat, idx) => ({
      name: cat,
    ...
    if (data.length === 0) {
      return [{ name: "No Expenses", value: 1, fill: "rgba(212, 175, 55, 0.15)" }];
    }
    return data;
  };
  ...
  // Get data for income generated by vehicle
  const getIncomeByVehicleData = () => {
    const txsToUse = selectedVehicleId ? (filteredTxs.length > 0 ? filteredTxs : vehicleTxs) : transactions;
    ...
    incomeTxs.forEach(t => {
      ...
      const name = v ? v.vehicleNumber : 'Unknown';
      vehicleTotals[name] = (vehicleTotals[name] || 0) + t.amount;
    });

    const colors = ['#D4AF37', '#FCF6BA', '#AA771C', '#BCAE9A', '#7A7264', '#E6C24C', '#A59E92'];
    const data = Object.keys(vehicleTotals).map((name, idx) => ({
      name,
      value: vehicleTotals[name],
      fill: colors[idx % colors.length]
    }));

    if (data.length === 0) {
      return [{ name: "No Income", value: 1, fill: "rgba(212, 175, 55, 0.15)" }];
    }
    return data;
  };

  const incomeByVehicleData = getIncomeByVehicleData();

  // Get data for total cash flow status (Income vs Expense vs Pending)
  ...
  const getRoundChartData = () => {
    if (roundChartMode === 'vehicles') return incomeByVehicleData;
    if (roundChartMode === 'cashflow') return cashFlowData;
    return expenseBreakdownData;
  };

  const roundChartData = getRoundChartData();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="reports-charts-row">
        {/* Trend Chart (Left 50%) */}
        <div className="card trend-chart-card" style={{ display: 'flex', flexDirection: 'column', minHeight: '380px', overflow: 'hidden' }}>
          <h3 className="card-title">Income vs Expenses Trend</h3>
          <div 
            ref={chartScrollRef}
            className="trend-chart-scroll-container"
            style={{ width: '100%', marginTop: '16px', paddingBottom: '8px' }}
          >
        ...
        {/* Donut Chart (Right 50%) */}
        <div className="card donut-chart-card" style={{ display: 'flex', flexDirection: 'column', minHeight: '380px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
            <h3 className="card-title" style={{ margin: 0 }}>
              {roundChartMode === 'expenses' && 'Expense Breakdown'}
              {roundChartMode === 'vehicles' && 'Income by Vehicle'}
              {roundChartMode === 'cashflow' && 'Total Cash Flow'}
            </h3>
            <select 
              className="form-select form-select-sm" 
              style={{ width: '180px', padding: '6px 12px', fontSize: '0.8rem', height: '32px', border: '1px solid rgba(212, 175, 55, 0.25)', borderRadius: '4px', backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)' }}
              value={roundChartMode}
              onChange={e => setRoundChartMode(e.target.value as any)}
            >
              <option value="expenses">Expense Categories</option>
              <option value="vehicles">Income by Vehicle</option>
              <option value="cashflow">Total Cash Flow</option>
            </select>
          </div>
          <div style={{ flexGrow: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', height: '300px', marginTop: '16px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={roundChartData}
                  ...
                  dataKey="value"
                >
                  {roundChartData.map((entry: any, index: number) => (
